...
function Dashboard({ user, projects }) {
    ...
    useEffect(()=>{if (user?.student) {console.log("STUDENT!")}},[user])
    return (
        <Layout>
            <Side sidebaritems={sidebaritems} setPage={setPage}>
            {(page === 'Profile'  ) ? <>{user?.student ? <Profile type="Student" user={user} editing={true}/>: <Profile type="Faculty" user={user} editing={true}/>}</>:<></>}
